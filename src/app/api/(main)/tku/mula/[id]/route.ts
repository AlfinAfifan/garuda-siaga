...
export const DELETE = async (req: Request, { params }: { params: Promise<{ id: string }> }) => {
  try {
    ...
    if (existingData.tata) {
      return new NextResponse('Cannot delete Bantu data when Tata is present', { status: 400 });
    }

    const deletedData = await Tku.findByIdAndUpdate(id, { bantu: false, sk_bantu: '', date_bantu: null });
