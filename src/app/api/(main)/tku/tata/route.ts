...
export const GET = async (req: NextRequest) => {
  try {
    ...
    // Filter awal hanya untuk tata
    const initialMatchStage: any = { tata: true, is_delete: 0 };
...
export const POST = async (req: Request) => {
  try {
    ...
    if (!existingData || !existingData.mula || !existingData.bantu) {
      return new NextResponse('Member not found or not eligible for Tata', { status: 404 });
    }
    if (existingData.tata) {
    ...
    // Ambil nomor urut SK
    const lastTku = await Tku.findOne({ tata: true }, {}, { sort: { createdAt: -1 } });
    let nomorUrut = 1;
    if (lastTku && lastTku.sk_tata) {
      const match = lastTku.sk_tata.match(/^(\d{5})/);
      if (match) {
    ...
    // Format SK: nomor_urut_5_digit/TKU-TATA/gudep-A/tahun
    ...
    const sk_tata = `${nomorUrutStr}/TKU-TATA/${gudep}-A/${tahun}`;
    const date_tata = new Date().toISOString().split('T')[0];

    const updatedData = await Tku.findByIdAndUpdate(id, { sk_tata, date_tata, tata: true }, { new: true });
